...
class PipelineStageSerializer(serializers.ModelSerializer):
    class Meta: model = PipelineStage; fields = '__all__'
class PipelineSerializer(serializers.ModelSerializer):
    stages = PipelineStageSerializer(many=True, read_only=True)
    class Meta: model = Pipeline; fields = '__all__'
class BuildStageRecordSerializer(serializers.ModelSerializer):
    stage_name = serializers.CharField(source='stage.name', read_only=True)
    class Meta: model = BuildStageRecord; fields = '__all__'
class BuildRecordSerializer(serializers.ModelSerializer):
    ...
    class Meta: model = BuildRecord; fields = '__all__'
